...
public static partial class SiftRuleCatalog
{
    public static IReadOnlyList<SiftRule> Default { get; } =
    [
    ...
    private static SiftRule Rule(string id, string name, string severity, string confidence, Regex pattern, string? secretGroup = null, Func<string, bool>? validator = null)
        => new(id, name, severity, confidence, pattern, secretGroup, validator);

    [GeneratedRegex(@"-----BEGIN (?:RSA |EC |OPENSSH |DSA )?PRIVATE KEY-----", RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex PrivateKeyPattern();
    [GeneratedRegex(@"\b(?:AKIA|ASIA)[A-Z0-9]{16}\b", RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex AwsAccessKeyPattern();
    [GeneratedRegex(@"\b(?:aws_secret_access_key|awsSecretAccessKey)\b\s*[:=]\s*[""']?(?<secret>[A-Za-z0-9/+=]{40})", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex AwsSecretKeyPattern();
    [GeneratedRegex(@"\b(?:gh[pousr]_[A-Za-z0-9]{36,255}|github_pat_[A-Za-z0-9_]{20,255})\b", RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex GitHubTokenPattern();
    [GeneratedRegex(@"\bxox[baprs]-[A-Za-z0-9-]{10,200}\b", RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex SlackTokenPattern();
    [GeneratedRegex(@"\bsk-(?:proj-)?[A-Za-z0-9_-]{20,200}\b", RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex OpenAiKeyPattern();
    [GeneratedRegex(@"\b[rs]k_(?:live|test)_[A-Za-z0-9]{16,}\b", RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex StripeKeyPattern();
    [GeneratedRegex(@"\beyJ[A-Za-z0-9_-]{5,}\.[A-Za-z0-9_-]{5,}\.[A-Za-z0-9_-]{5,}\b", RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex JsonWebTokenPattern();
    [GeneratedRegex(@"\b(?:password|pwd)\s*=\s*[""']?(?<secret>[^;\s,""']{4,})", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex ConnectionPasswordPattern();
    [GeneratedRegex(@"\b(?:api[_-]?key|client[_-]?secret|access[_-]?token|auth[_-]?token|password|passwd)\b\s*[:=]\s*[""']?(?<secret>[A-Za-z0-9_./+=:@-]{8,})", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex SecretAssignmentPattern();
    [GeneratedRegex(@"\b[a-z][a-z0-9+.-]*://[^\s/:@]+:(?<secret>[^\s/@]{4,})@[^\s]+", RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex BasicAuthUriPattern();
    [GeneratedRegex(@"\b(?:\d[ -]*?){13,19}\b", RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex PaymentCardPattern();
    [GeneratedRegex(@"\b[A-Z]{2}\d{2}(?:[ ]?[A-Z0-9]){11,30}\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant, matchTimeoutMilliseconds: 250)]
    private static partial Regex IbanPattern();

    public static bool IsValidPaymentCard(string value)
    {
